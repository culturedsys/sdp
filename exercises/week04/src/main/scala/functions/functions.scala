...
object Funcs {
     ...
     */
     ...
     def reverse[A](ls: List[A]): List[A] = foldLeft(ls, Nil : List[A])((acc, elem) => elem :: acc)
     def flatten[A](ls: List[List[A]]): List[A] = foldLeft(ls, Nil : List[A])((acc, otherList) => acc ++ otherList)
     ...
     * elements of ls.
     */
    def flatMap[A,B](ls: List[A])(f: A => List[B]): List[B] = ls match { // could just be written as flatten(map(ls)(f)), but that would traverse the list twice
        case Nil => Nil
     ...
     */
     def maxAverage(ls: List[(Double, Double)]): Double = {
        def max(pair: (Double, Double)) = pair match {
     ...
     */
     def variance(ls: List[Double]): Double = {
        ...
        sum(map(ls)(v => (v - mean) * (v - mean))) / count
    }
}
